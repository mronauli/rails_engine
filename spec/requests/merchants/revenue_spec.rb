...
describe "Total revenue across date range API" do
  before(:each) do
    ...
    @item_5 = Item.create(name: "Super Disposable Batteries", description: "Dispose at your convenience.", unit_price: 2391, merchant: @merchant_3)
    ...
    @invoice_item_6 = InvoiceItem.create(item: @item_5, invoice: @invoice_4, quantity: 11, unit_price: @item_5.unit_price)
    @transaction_1 = Transaction.create(credit_card_number: 4444555566667777, credit_card_expiration_date: '', result: "success", invoice: @invoice_1, updated_at: Date.new(2020, 3, 10))
    @transaction_2 = Transaction.create(credit_card_number: 4444555566668888, credit_card_expiration_date: '', result: "success", invoice: @invoice_2, updated_at: Date.new(2020, 3, 13))
    @transaction_3 = Transaction.create(credit_card_number: 4444555566667777, credit_card_expiration_date: '', result: "success", invoice: @invoice_3, updated_at: Date.new(2020, 3, 20))
    @transaction_4 = Transaction.create(credit_card_number: 4444555566667777, credit_card_expiration_date: '', result: "success", invoice: @invoice_4, updated_at: Date.new(2020, 3, 23))
  end
  it "sends total revenue for particular merchant" do
    get "/api/v1/merchants/#{@merchant_3.id}/revenue"

    expect(response).to be_successful

    revenue = json
    expect(revenue[:data][:attributes][:revenue]).to eq(263.01)
  end
end
